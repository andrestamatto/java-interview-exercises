...
public final class JpaOrderSummaryStore implements OrderSummaryStore {
  ...
  public List<OrderSummary> loadSummaries() {
    return entityManager
        .createQuery(
            "select distinct purchaseOrder from purchase_orders purchaseOrder left join fetch purchaseOrder.lines order by purchaseOrder.id",
            JpaPurchaseOrder.class)
        .getResultList()
        .stream()
        .map(
            order ->
                new OrderSummary(
                    ...
                    order.customer(),
                    order.lines().stream()
                        .map(line -> Math.multiplyExact(line.quantity(), line.unitPriceCents()))
